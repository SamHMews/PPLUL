...
assert.equal(await p.locator('#remaining-count').innerText(),'0 of 8 remaining');assert.equal(await p.locator('.exercise-card.is-complete').count(),8);await p.reload();await p.locator('.is-front').waitFor();assert.equal(await p.locator('#remaining-count').innerText(),'0 of 8 remaining');await p.locator('.is-front').focus();await p.keyboard.press('ArrowRight');await p.waitForTimeout(600);assert.equal(await p.locator('.is-front h2').innerText(),'Bench Press');await p.getByRole('button',{name:'Undo completion',exact:true}).click();await p.waitForTimeout(100);assert.equal(await p.locator('#remaining-count').innerText(),'1 of 8 remaining');
