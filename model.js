...
export function migrateState(input){
 ...
 for(const day of DAYS){const session=s.active.days[day];
  for(const e of session.entries.filter(removed)){
   ...
   if(e.completed||e.weight!==null||e.reps!==null||e.effort!==null){const sourceId='retired:'+s.active.id+':'+day+':'+e.id;if(!s.seed.some(x=>x.sourceId===sourceId))s.seed.push({...structuredClone(e),day,date:e.completedAt,source:'programme_migration',sourceId,sets_source:'routine'});}
 ...
 for(const e of s.active.days.Lower.entries){if(e.id==='deadlift')e.name='Deadlift';if(e.id==='abs')e.name='Cable Crunches';}
 s.programmeVersion=4;
